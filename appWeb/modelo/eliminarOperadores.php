...
function obtenerUltimoReg(){
	global $conexion;
	$ultimoId = "SELECT MAX(id_operadores) FROM operadores";// obtener ultimo id
...
function eliminarClientes($idCliente){
	...
	global $conexion;
	$delete = "DELETE FROM operadores WHERE id_operadores = $idCliente"; //se borra fila
...
function actualizarIdRegistros($idCliente, $ultimoReg ){
	...
	while ($num <= $ultimoReg) {
	 //	echo 'newID: '.$newId;
		...
		//echo 'num: '.$num;
		$update = "UPDATE operadores SET id_operadores=$newId WHERE id_operadores=$num";
...
function actualizarAutoIncrement($ultimoReg){
	...
	$ai = $ultimoReg - 1;
	//echo 'AI: '.$ai;
	$resetAI="ALTER TABLE operadores AUTO_INCREMENT = $ai";
